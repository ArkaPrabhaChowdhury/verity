...
- 2026-07-23 — Implemented Supabase Postgres as an optional hosted-durability target. The tracked migration places application tables in a private `verity` schema with RLS enabled and no browser-client grants; live project provisioning was deferred, so SQLite remains the current runtime.
- 2026-07-23 — Retained SQLite as the zero-setup and current runtime; setting `VERITY_DATABASE_URL` switches the repository to the optional Postgres adapter.
- 2026-07-29 — Corrected documentation and portfolio language that implied Supabase was live. The adapter and migration are implemented, but no Verity Supabase project has been provisioned.
