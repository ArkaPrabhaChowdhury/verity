...
export function DetailsPanel({ run }: { run?: Run }) {
  ...
  return (
    <section className="panel-shell details-panel" aria-labelledby="details-title">
      ...
      <div className="metric-grid"><Metric label="Status" value={run.status} /><Metric label="Mode" value={run.options?.replan_enabled ? "Critic re-plan" : "No re-plan"} /><Metric label="Latency" value={`${(run.metadata.total_latency_ms / 1000).toFixed(1)}s`} /><Metric label="Tokens" value={run.metadata.total_tokens.toLocaleString()} /><Metric label="Searches" value={String(run.metadata.search_queries)} /><Metric label="Cost" value={`$${run.metadata.estimated_cost_usd.toFixed(4)}`} /></div>
