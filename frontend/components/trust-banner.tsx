...
export function TrustBanner({ run }: { run?: Run }) {
  ...
  return (
    <section className={`trust-banner ${trust.status}`} aria-label={`Trust assessment: ${trust.status}`}>
      ...
      <dl><div><dt>Fully supported</dt><dd>{trust.successful_findings}</dd></div><div><dt>Usable with gaps</dt><dd>{trust.partial_findings}</dd></div><div><dt>Failed</dt><dd>{trust.failed_findings}</dd></div><div><dt>Source domains</dt><dd>{trust.independent_domains}</dd></div></dl>
